...
This runs the SD-app-style binaries in `vendor/images`, writes PNG/trace artifacts under `build/vendor-emu`, and keeps going after individual failures. It is an investigation target, not a passing compatibility suite yet. The current emulator gets those binaries through vector loading, early reset/clock setup, targeted Boot ROM helper lookup, SIO divider/spinlock setup, selected RAM helper paths, a low-flash boot2 return stub, a flash buffer rooted at `0x10000000`, erased-flash reads beyond the loaded `.bin`, optional persistent flash-state files, a deterministic XIP `0x4b` unique-ID response, emulated Boot ROM flash erase/program helpers, SPI APB aliases, 16-bit LCD pixel decoding, a minimal UART0 status path, minimal PIO FIFO/debug status, hardware-calibrated raw timer reads, and conservative timer-wait acceleration. MP3 and PicoMite currently exit cleanly without LCD output; Lua reaches coherent 6x8 RGB565 glyph-cell windows before budgeting later near `0x1005ad68`, but still needs enough runtime progress to compare a full prompt against the real black/white/yellow Lua screen; uLisp reaches the interactive prompt (`uLisp 4.8f`, `22280> _`) and then budgets in the keyboard I2C transfer loop near `0x10044cd4`; MicroPython still loops in a null storage/metadata path near `0x10059ce8`; NES has moved past the earlier divider-wrapper null callback, a PIO TX FIFO/debug wait, and an SDK timer wait, and now budgets later near `0x10043b34` before LCD output. The emulator records LCD `INVON`/`INVOFF` state for traces, but does not invert PNG framebuffer pixels because the non-inverted PNG output matches the observed PicoCalc display.
...
The Thumb probe covers a previously missing Cortex-M0+ `LDRH` immediate decode. The interrupt probe exercises `SVC`, `SysTick`, exception return, and NVIC IRQ0. The DMA probe transfers a framebuffer block to SPI1 through the DMA MMIO path and records DMA trace events. The vendor-startup probe covers the current SDK-derived startup surface, including Boot ROM helper lookup, SIO divider/spinlocks, timer reads, I2C enable status, RTC active polling, SPI0 ready status, minimal PIO FIFO/debug status, and XIP SSI ready status.
