...
This runs the SD-app-style binaries in `vendor/images`, writes PNG/trace artifacts under `build/vendor-emu`, and keeps going after individual failures. It is an investigation target, not a passing compatibility suite yet. The current emulator gets those binaries through vector loading, early reset/clock setup, targeted Boot ROM helper lookup, SIO divider/spinlock setup, selected RAM helper paths, a low-flash boot2 return stub, erased-flash reads beyond the loaded `.bin`, a deterministic XIP `0x4b` unique-ID response, and no-op Boot ROM flash erase/program helpers. MP3 and PicoMite currently exit cleanly without LCD output; uLisp renders recognizable white text (`uLisp 4.8f`, a newline, and `_`) before a later budget stop; Lua reaches LCD writes but currently produces only unclassified colorful pixels along the first screen line; MicroPython passes RTC setup and copied flash helper paths before budgeting in application/data logic; NES reaches a divider-wrapper null callback before LCD output.
