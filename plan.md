...
- `make emu-vendor-probe` now runs those SD-app-style vendor binaries as an investigation suite, writes artifacts under `build/vendor-emu`, reports first LCD milestones, and keeps going after failures. Current status: MP3 and PicoMite exit cleanly without LCD output; uLisp renders recognizable white text (`uLisp 4.8f`, a newline, and `_`) before budgeting in RAM code; Lua reaches LCD writes through the SPI0 path but currently produces only unclassified colorful pixels along the first screen line; MicroPython passes RTC setup, erased-flash reads, and copied flash helper paths before budgeting in application/data logic; NES still stops before any LCD command through a divider-wrapper null callback.
...
| Feature | Status | Explanation |
| --- | --- | --- |
| Flash and RAM mapping | Partially implemented | Maps firmware bytes at the SD app flash base and maps RP2040 SRAM at `0x20000000` with the current firmware stack top at `0x20042000`. A tiny low-flash boot2 return stub is exposed at `0x10000000` for SDK copied restore-XIP helpers, and unread flash bytes beyond the loaded `.bin` read back as erased `0xff`. The flash buffer is fixed-size and tailored to SD-app binaries. |
...
| SPI0/SPI1 MMIO | Partially implemented | Models SPI0 and SPI1 control/status/data registers through one minimal SPI state machine, including TX/RX FIFO levels, busy state, overrun state, and SPI0/SPI1 TX DREQ paths. Timing is approximate and tuned for useful firmware execution, not cycle-accurate SPI behavior. |
...
| Complete XIP SSI and flash controller | Not yet implemented | The firmware image is mapped as memory directly and a minimal XIP SSI status/data-register stub exists. XIP cache, general SSI command responses, real flash erase/program, and cache maintenance behavior are absent. |
...
| --- | --- | --- | --- |
| 1 | Vendor runtime budget diagnosis | In progress | Call/branch, Boot ROM, unknown-MMIO, XIP, LCD milestone traces, and visual inspection now classify the active blockers. Current frontier: MP3 and PicoMite exit cleanly without LCD output; uLisp renders `uLisp 4.8f`, newline, and `_` before a later budget stop; Lua shows only unclassified colorful first-line pixels; MicroPython budgets near `0x10059d44` after RTC, erased-flash reads, and copied flash helper paths; NES reaches a divider-wrapper null callback before LCD output. |
| 2 | MicroPython application/data loop | In progress | The original null flash-storage reads now return erased `0xff`, which moves MicroPython to the fallback branch in the same file/data routine. The remaining loop appears to be application-level missing filesystem or persistent flash content rather than simple unknown MMIO. |
| 3 | uLisp post-SysTick RAM loop | In progress | The repeated SysTick exception starvation was fixed by giving thread mode a tick after exception return. uLisp now reaches LCD commands and renders recognizable white text (`uLisp 4.8f`, a newline, and `_`), then budgets in a copied RAM loop near `0x2000010a`. |
| 4 | Lua post-SPI0 LCD loop | In progress | Adding SPI0 support moved Lua past the `0x4003c000` status loop and into LCD output, but the visible image is currently only colorful pixels along the first screen line with no known pattern. It now budgets later in copied RAM around `0x200002ba`, so the next Lua work is to classify whether this is a pixel-format/address-window issue or the next peripheral wait. |
| 5 | NES divider-wrapper null callback | In progress | NES's late null branch is identified as the clean-path callback slot in its SIO divider wrapper at `0x10043b72`/`0x10043b76`, not a missing Boot ROM helper. Determine whether the zero callback comes from missing game/media data, skipped runtime table initialization, or divide-by-zero handling. |
| 6 | PicoMite clean-exit meaning | In progress | PicoMite exercises repeated no-op Boot ROM flash erase/program paths and later peripheral setup, then exits/halts without LCD milestones. The clean exit is not an emulator crash; next check whether persistent flash writes or companion files are required for visible output. |
| 7 | Improve SD/media emulation path | Next | Determine whether MP3, NES, PicoMite, or MicroPython need companion SD files or persistent flash content, then sketch the smallest host-backed block, file, or flash-storage interface needed to let those apps reach their first screen. |
