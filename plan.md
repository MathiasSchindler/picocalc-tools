...
- `make emu-vendor-probe` now runs those SD-app-style vendor binaries as an investigation suite, writes artifacts under `build/vendor-emu`, reports first LCD milestones, and keeps going after failures. Current status: MP3 and PicoMite exit cleanly without LCD output; uLisp renders recognizable white text (`uLisp 4.8f`, a newline, and `_`) and now budgets in a later allocator/idle scan; Lua reaches LCD writes through the SPI path but currently produces only a small RGB565-decoded stripe from repeated 6x8 windows across row 0; MicroPython passes RTC setup, erased-flash reads, and copied flash helper paths before budgeting in a null storage/metadata loop around its SIO divider wrapper; NES still stops before any LCD command through a divider-wrapper null callback.
...
| SPI0/SPI1 MMIO | Partially implemented | Models SPI0 and SPI1 control/status/data registers through one minimal SPI state machine, including TX/RX FIFO levels, busy state, overrun state, SPI DMACR writes, APB atomic aliases, and SPI0/SPI1 TX DREQ paths. Timing is approximate and tuned for useful firmware execution, not cycle-accurate SPI behavior. |
| LCD SPI command stream | Partially implemented | Interprets the LCD command/data stream for address windows and memory writes, especially `CASET`/`RASET`/`RAMWR` style drawing. It handles the local RGB666-style byte stream and vendor RGB565 (`COLMOD=0x55`) writes. The full LCD controller command set is not implemented. |
| I2C1 MMIO | Partially implemented | Models the I2C1 registers used by the PicoCalc keyboard driver, including target address, data command, status, enable status, raw interrupt, stop clear, TX abort clear, threshold/DMA registers, APB atomic aliases, and timing-related configuration registers. FIFO/timing behavior is approximate. |
...
| Minimal Boot ROM helper stubs | Partially implemented | Emulates the low-memory RP2040 ROM table pointers and lookup routine for the vendor SDK helpers observed so far: bit helpers, memory helpers, data-table lookups, and flash erase/program helpers that mutate the emulated flash buffer. Boot ROM boot flow, real flash timing/status, USB boot, and unobserved ROM helpers are not implemented. |
| Minimal timer readback | Partially implemented | Exposes raw timer high/low reads used by local diagnostics and vendor delay code. Low timer reads are scaled from the emulator cycle counter as `g_cycles / 100` based on real PicoCalc benchmark data. Alarms, IRQs, pause/debug behavior, and full timer register semantics are not modeled. |
...
| Vendor PicoCalc binary compatibility suite | Partially implemented | `make emu-vendor-probe` runs Lua, MicroPython, MP3 player, NES, PicoMite, and uLisp SD-app `.bin` files from `vendor/images` with bounded step counts, traces, LCD milestone reports, and PNG artifacts. With the earlier fast raw-timer model, Lua/uLisp/PicoMite reached deeper LCD or clean-exit frontiers; after calibrating `TIMERAWL` toward hardware microseconds, the current 2M-step budget is too small for some SDK timer waits, so Lua, MicroPython, PicoMite, and uLisp now stop earlier unless given a larger budget or timer-wait acceleration. MP3 still exits cleanly and NES still reaches its divider-wrapper null callback. The BIOS and keyboard `.bin` files under `vendor/PicoCalc/Bin` remain separate references with different vectors/load bases. |
...
| --- | --- | --- | --- |
| 1 | Vendor runtime budget diagnosis | In progress | Implemented SPI APB aliases/DMACR, I2C aliases/threshold/DMA registers, UART0 status, complete Boot ROM lookup-pointer word reads, RGB565 LCD decoding, decoded LCD window tracing, flash-rooted image mapping, emulated Boot ROM flash erase/program mutation, optional persistent flash-state files, and a raw-timer scale calibrated from PicoCalc benchmark output. Current caveat: the more realistic timer scale makes the fixed 2M-step vendor probe budget too small for several SDK timer waits, so the next vendor-runtime work is timer-wait acceleration or budget policy rather than reverting to the old fast timer. |
| 2 | MicroPython storage/metadata loop | In progress | Erased flash and complete Boot ROM pointer reads moved MicroPython through the earlier flash-helper path, but it now repeatedly dereferences a null metadata/storage pointer at field `+0x28` and calls the SIO divider wrapper with no LCD output. This is now classified as missing persistent filesystem/flash metadata rather than a simple peripheral readiness loop. |
| 3 | uLisp allocator/idle scan | In progress | SysTick starvation and the copied-RAM UART0 poll are both resolved: uLisp reaches LCD commands, renders recognizable white text (`uLisp 4.8f`, a newline, and `_`), and then budgets in a later flash allocator/idle scan near `0x10032686`. |
| 4 | Lua RGB565 first-stripe output | In progress | SPI0/SPI1 aliases, RGB565 RAMWR decoding, and decoded LCD window tracing are implemented. Lua repeatedly draws 6x8 windows from `x=0..0x71`, `y=0..7`, producing 182 nonblack RGB565-decoded pixels and budgeting later in copied RAM around `0x200002ba`; the next Lua work is to determine why it never advances beyond the first text row. |
| 5 | NES divider-wrapper null callback | In progress | NES's late null branch is identified as the clean-path callback slot in its SIO divider wrapper at `0x10043b72`/`0x10043b76`, not a missing Boot ROM helper. A speculative always-dirty SIO divider state did not move this frontier; the likely remaining causes are skipped runtime table initialization, missing media/game data, or a divide-by-zero callback path. |
| 6 | PicoMite clean-exit meaning | In progress | PicoMite exercises repeated no-op Boot ROM flash erase/program paths and later reset/watchdog-style peripheral setup, then exits/halts cleanly without LCD milestones. The clean exit is not an emulator crash; the likely next dependency is persistent flash contents or companion files rather than a missing LCD path. |
