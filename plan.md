...
| Minimal XIP SSI status and ID command | Partially implemented | Returns nonbusy TX/RX-ready status for the XIP SSI status register used by copied RAM flash helper stubs, traces XIP SSI reads/writes with `--trace-kinds=xip`, and returns a fixed deterministic response for the observed DR0 `0x4b` read-unique-ID command. XIP cache, full SSI command execution, flash erase/program behavior, and cache maintenance are not modeled. |
...
| Vendor PicoCalc binary compatibility suite | Partially implemented | `make emu-vendor-probe` runs Lua, MicroPython, MP3 player, NES, PicoMite, and uLisp SD-app `.bin` files from `vendor/images` with bounded step counts, traces, LCD milestone reports, and PNG artifacts. MP3 and PicoMite now exit cleanly in the emulator without LCD output. uLisp reaches LCD commands and renders recognizable white text (`uLisp 4.8f`, a newline, and `_`) before budgeting in a later allocator/idle scan; a scripted uLisp expression reaches app input and produces visible output, but not the expected expression echo/result yet. Lua reaches LCD command `0xe0` and RGB565-decoded nonblack framebuffer pixels from repeated 6x8 windows across row 0; MicroPython passes RTC setup, erased-flash reads, copied flash helper paths, and emulated flash helper mutation before budgeting in a null storage/metadata loop around its SIO divider wrapper; NES invalid-executes through a divider-wrapper null callback before LCD output. The BIOS and keyboard `.bin` files under `vendor/PicoCalc/Bin` remain separate references with different vectors/load bases. |
...
| --- | --- | --- | --- |
| 1 | Vendor runtime budget diagnosis | In progress | Implemented SPI APB aliases/DMACR, I2C aliases/threshold/DMA registers, UART0 status, complete Boot ROM lookup-pointer word reads, RGB565 LCD decoding, decoded LCD window tracing, flash-rooted image mapping, emulated Boot ROM flash erase/program mutation, and optional persistent flash-state files. Current frontier: MP3 and PicoMite exit cleanly without LCD output; uLisp renders `uLisp 4.8f`, newline, and `_` before a later allocator/idle scan; Lua shows a small RGB565-decoded stripe from repeated 6x8 row-0 windows; MicroPython budgets in a null storage/metadata loop around its SIO divider wrapper; NES reaches a divider-wrapper null callback before LCD output. |
