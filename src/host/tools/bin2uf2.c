...
static int write_wrapped_uf2(const char *boot2_path, const char *input_path, const char *output_path, uint32_t app_base, uint32_t family_id) {
    ...
    for (block_no = 0; block_no < blocks; ++block_no) {
        ...
        if (block_no == 0) {
            ...
            put32le(payload + 4, RP2040_FLASH_BASE + WRAPPER_VECTOR_BLOCK_OFFSET + 8u + 1u);
            memcpy(payload + 8, g_wrapper_code, sizeof(g_wrapper_code));
            put32le(payload + 8 + 16, app_base);
        } else {
            uint32_t app_pos = (block_no - 2u) * UF2_PAYLOAD_SIZE;
    ...
    printf("%s: %u-byte app wrapped at 0x%08x -> %u sparse UF2 blocks\n", output_path, app_size, app_base, blocks);
    free(app_data);
    return 0;
}
