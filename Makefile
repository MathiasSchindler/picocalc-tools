...
FONT1_DIR := $(BUILD_DIR)/font-1bpp
FONTGEN_1BPP_HEADER := $(FONT1_DIR)/picocalc_cascadia_8x14.h
FONTGEN_1BPP_PREVIEW := $(FONT1_DIR)/picocalc_cascadia_8x14.ppm
...
EMU_HASH_AEABI_DOUBLE := 0xc5fa6567
...
PICOW_WIFI_DIAG_EMU_STEPS ?= 50000000
...
BARE_SOLVE_CORE_1BPP_PICOLINK_BIN := $(BARE_1BPP_DIR)/bare_solve_core_1bpp_picolink.bin
...
BARE_LTO_LDFLAGS ?= $(BARE_LDFLAGS) -flto=auto -mcpu=cortex-m0plus -mthumb
BARE_LTO_RELOC_LDFLAGS ?= -r -nostdlib -flto=auto -flinker-output=nolto-rel -mcpu=cortex-m0plus -mthumb
BARE_SOLVE_LTO_ELF := $(BARE_LTO_DIR)/bare_solve_lto.elf
BARE_SOLVE_LTO_BIN := $(BARE_LTO_DIR)/bare_solve_lto.bin
BARE_CUBE_LTO_RELOC := $(BARE_LTO_DIR)/bare_cube_lto_reloc.o
...
$(FONTGEN_1BPP_HEADER) $(FONTGEN_1BPP_PREVIEW) &: $(FONTGEN) $(FONTGEN_INPUT) | $(FONT1_DIR)
	$(FONTGEN) $(FONTGEN_INPUT) $(FONTGEN_1BPP_HEADER) $(FONTGEN_1BPP_PREVIEW) 1

font-cascadia: $(FONTGEN_HEADER) $(FONTGEN_PREVIEW)

font-cascadia-1bpp: $(FONTGEN_1BPP_HEADER) $(FONTGEN_1BPP_PREVIEW)
...
bin-emu-solve-core-1bpp-picolink: $(BIN_EMU) $(BARE_SOLVE_CORE_1BPP_PICOLINK_BIN)
	$(BIN_EMU) $(BARE_SOLVE_CORE_1BPP_PICOLINK_BIN) $(EMU_DIR)/bare_solve_core_1bpp_picolink.png '6x-3=0\n\x04' --symbols=$(BARE_SOLVE_CORE_1BPP_PICOLINK_MAP) --expect-hash=$(EMU_HASH_SOLVE_CORE_1BPP_PICOLINK)
...
sdk-hello-uf2: font-cascadia
	PICO_SDK_PATH=$(PICO_SDK_PATH) cmake -S . -B $(SDK_HELLO_BUILD_DIR) -DPICO_BOARD=pico_w
	cmake --build $(SDK_HELLO_BUILD_DIR) --target picocalc_hello_flash -j2
	@ls -lh $(SDK_HELLO_BIN) $(SDK_HELLO_UF2)
...
emu-deterministic-tests: emu-replay-manifest-check $(BIN_EMU) $(BARE_HELLO_BIN) $(BARE_GRAPHICS_BIN) $(BARE_SOLVE_BIN) $(BARE_INTERRUPT_BIN) $(BARE_DMA_BIN) $(BARE_THUMB_BIN) $(BARE_AEABI_DOUBLE_BIN) $(BARE_VENDOR_STARTUP_BIN) $(EMU_SOLVE_REPLAY)
...
$(BARE_LTO_DIR):
	mkdir -p $(BARE_LTO_DIR)

$(BARE_1BPP_DIR):
	mkdir -p $(BARE_1BPP_DIR)
...
$(BARE_UF2_MENU_DIR)/%.uf2: $(BARE_LTO_DIR)/%.bin $(BIN2UF2) | $(BARE_UF2_MENU_DIR)
	$(BIN2UF2) --flash-start "$(PICOCALC_BOOT2_UF2)" $< $@ $(PICOCALC_APP_FLASH_BASE) $(RP2040_UF2_FAMILY_ID)

$(BARE_UF2_MENU_DIR)/%.uf2: $(BARE_1BPP_DIR)/%.bin $(BIN2UF2) | $(BARE_UF2_MENU_DIR)
	$(BIN2UF2) --flash-start "$(PICOCALC_BOOT2_UF2)" $< $@ $(PICOCALC_APP_FLASH_BASE) $(RP2040_UF2_FAMILY_ID)

$(BARE_UF2_VARIANTS_DIR)/%.uf2: $(BARE_DIR)/%.bin $(BIN2UF2) | $(BARE_UF2_VARIANTS_DIR)
	$(BIN2UF2) --flash-start "$(PICOCALC_BOOT2_UF2)" $< $@ $(PICOCALC_APP_FLASH_BASE) $(RP2040_UF2_FAMILY_ID)

$(BARE_UF2_VARIANTS_DIR)/%.uf2: $(BARE_LTO_DIR)/%.bin $(BIN2UF2) | $(BARE_UF2_VARIANTS_DIR)
	$(BIN2UF2) --flash-start "$(PICOCALC_BOOT2_UF2)" $< $@ $(PICOCALC_APP_FLASH_BASE) $(RP2040_UF2_FAMILY_ID)

$(BARE_UF2_VARIANTS_DIR)/%.uf2: $(BARE_1BPP_DIR)/%.bin $(BIN2UF2) | $(BARE_UF2_VARIANTS_DIR)
	$(BIN2UF2) --flash-start "$(PICOCALC_BOOT2_UF2)" $< $@ $(PICOCALC_APP_FLASH_BASE) $(RP2040_UF2_FAMILY_ID)

bare-uf2-menu: $(BARE_MENU_UF2S)
	@find $(BARE_UF2_DIR) -maxdepth 1 -name '*.uf2' -delete
	@ls -lh $(BARE_MENU_UF2S)

bare-uf2-variants: $(BARE_VARIANT_UF2S)
	@find $(BARE_UF2_DIR) -maxdepth 1 -name '*.uf2' -delete
	@ls -lh $(BARE_VARIANT_UF2S)

bare-uf2-all: $(BARE_UF2S)
	@find $(BARE_UF2_DIR) -maxdepth 1 -name '*.uf2' -delete
	@ls -lh $(BARE_MENU_UF2S)
	@ls -lh $(BARE_VARIANT_UF2S)
...
$(BARE_LTO_DIR)/%.o: $(PICOCALC_BARE_SRC_DIR)/%.c | $(BARE_LTO_DIR)
	$(BARE_CC) $(BARE_LTO_CFLAGS) -c $< -o $@

$(BARE_1BPP_DIR)/picocalc_lcd_bare.o: $(PICOCALC_BARE_SRC_DIR)/picocalc_lcd_bare.c $(FONTGEN_1BPP_HEADER) $(PICOCALC_BARE_SRC_DIR)/picocalc_font.h | $(BARE_1BPP_DIR)
	$(BARE_CC) -I$(FONT1_DIR) $(BARE_CFLAGS) -c $< -o $@

$(BARE_DIR)/%.o: $(PICOCALC_BARE_SRC_DIR)/%.S | $(BARE_DIR)
	$(BARE_CC) $(BARE_CFLAGS) -c $< -o $@

$(BARE_RUNTIME_ARCHIVE): $(BARE_AEABI_OBJ) $(BARE_AEABI_INT_OBJ) $(BARE_AEABI_DOUBLE_OBJ)
	$(BARE_AR) rcs $@ $(BARE_AEABI_OBJ) $(BARE_AEABI_INT_OBJ) $(BARE_AEABI_DOUBLE_OBJ)

$(BARE_DIR)/solve.o: src/solve.c | $(BARE_DIR)
	$(BARE_CC) $(BARE_CFLAGS) -c $< -o $@

$(BARE_DIR)/solve_core.o: src/solve.c | $(BARE_DIR)
	$(BARE_CC) $(BARE_CFLAGS) -DSOLVE_ENABLE_EXTENDED_MODES=0 -Wno-unused-function -c $< -o $@

$(BARE_LTO_DIR)/solve.o: src/solve.c | $(BARE_LTO_DIR)
	$(BARE_CC) $(BARE_LTO_CFLAGS) -c $< -o $@
...
$(BARE_SOLVE_PICOLINK_RELOC): $(BARE_SOLVE_OBJS) $(BARE_RUNTIME_ARCHIVE)
	$(BARE_CC) $(BARE_RELOC_LDFLAGS) $(BARE_SOLVE_OBJS) $(BARE_RUNTIME_ARCHIVE) $(BARE_LDLIBS) -o $@
...
$(BARE_SOLVE_CORE_PICOLINK_BIN): $(PICO_LINK) $(BARE_SOLVE_CORE_OBJS) $(BARE_RUNTIME_ARCHIVE)
	$(PICO_LINK) --stats --map=$(BARE_SOLVE_CORE_PICOLINK_MAP) -o $@ $(BARE_SOLVE_CORE_OBJS) $(BARE_RUNTIME_ARCHIVE)
	od -An -tx4 -N8 $@

bare-solve-core-picolink: $(BARE_SOLVE_CORE_PICOLINK_BIN)

$(BARE_SOLVE_CORE_1BPP_PICOLINK_BIN): $(PICO_LINK) $(BARE_SOLVE_CORE_1BPP_OBJS) $(BARE_RUNTIME_ARCHIVE) | $(BARE_1BPP_DIR)
	$(PICO_LINK) --stats --map=$(BARE_SOLVE_CORE_1BPP_PICOLINK_MAP) -o $@ $(BARE_SOLVE_CORE_1BPP_OBJS) $(BARE_RUNTIME_ARCHIVE)
	od -An -tx4 -N8 $@

bare-solve-core-1bpp-picolink: $(BARE_SOLVE_CORE_1BPP_PICOLINK_BIN)
...
$(BARE_SOLVE_LTO_BIN): $(BARE_SOLVE_LTO_ELF)
	$(BARE_OBJCOPY) -O binary $< $@
	$(BARE_SIZE) $<
	od -An -tx4 -N8 $@

bare-solve-lto: $(BARE_SOLVE_LTO_BIN)
...
$(BARE_CUBE_LTO_RELOC): $(BARE_CUBE_LTO_OBJS)
	$(BARE_CC) $(BARE_LTO_RELOC_LDFLAGS) $(BARE_CUBE_LTO_OBJS) $(BARE_LDLIBS) -o $@

$(BARE_CUBE_LTO_PICOLINK_BIN): $(PICO_LINK) $(BARE_CUBE_LTO_RELOC)
	$(PICO_LINK) --stats --map=$(BARE_CUBE_LTO_PICOLINK_MAP) -o $@ $(BARE_CUBE_LTO_RELOC)
	od -An -tx4 -N8 $@

bare-cube-lto-picolink: $(BARE_CUBE_LTO_PICOLINK_BIN)
...
$(BARE_AEABI_DOUBLE_BIN): $(BARE_AEABI_DOUBLE_ELF)
	$(BARE_OBJCOPY) -O binary $< $@
	$(BARE_SIZE) $<
	od -An -tx4 -N8 $@
